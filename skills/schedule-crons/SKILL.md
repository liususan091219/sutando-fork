...
3b. **Missed-fire catch-up (restart safety).** Session crons die with the session, so a daily job whose fire time passed before this registration silently skips its whole day (bit 2026-07-13: the 09:41 triage sweep was skipped by a 09:45 session restart — the owner noticed before the agent did). After step 3, for each registered entry whose cron is **daily or rarer** (fixed hour+minute — not `*/N`, not hourly):
   - Compute today's scheduled fire time from the cron expression.
   - If that time is already past AND the entry's run stamp `<workspace>/state/cron-lastrun/<name>` is missing or older than today's fire time, run the entry's prompt ONCE now as a make-up pass, then continue registration.
   - **Stamps make this detectable:** each daily entry's `prompt` should begin with a stamp line — `WS="$(bash scripts/sutando-config.sh workspace)"; mkdir -p "$WS/state/cron-lastrun" && date +%s > "$WS/state/cron-lastrun/<name>"` (see the `morning-briefing` entry in `crons.example.json`) — so catch-up can tell "already ran today" from "never fired". An entry with no stamp line in its prompt is skipped by catch-up (fail-safe: never risk double-running an unstamped job).
   - Sub-daily entries are excluded — their next natural tick recovers a missed fire within minutes (same rationale as the cron-gate cadence table below). This catch-up is a sanctioned, narrow exception to step 3's no-inline-fire rule: it recovers an owner-visible daily function instead of losing it until tomorrow, and the stamp check bounds it to at most one make-up run per job per day (a mid-session re-run finds a fresh stamp and does nothing — no 2026-06-03-style avalanche).
